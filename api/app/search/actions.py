...
async def add_stream_to_torbox(
    *,
    ...
    if identity.magnet is not None and identity.info_hash is not None:
        ...
        torrent_id = await _trigger_aiostreams_torbox_action(
...
async def _trigger_saved_aiostreams_action(
    *,
    ...
    try:
        ...
        return
    await repository.update_torbox_id(selection.stream_key, torrent_id)
...
async def _trigger_aiostreams_torbox_action(
    *,
    ...
    matched = _matching_download(stream, new_downloads) or (
        new_downloads[0] if len(new_downloads) == 1 else None
    )
    return _torrent_id(matched)
...
def _matching_download(
    ...
    expected_filename = filename.casefold() if isinstance(filename, str) else None
    expected_size = stream.behavior_hints.get("videoSize")
    for download in downloads:
        if _download_matches(download, expected_filename, expected_size):
            return download
    return None
...
def _download_matches(
    ...
    for raw_file in cast(list[object], files):
        ...
        name_matches = (
            ...
            and name.casefold().endswith(expected_filename)
        )
